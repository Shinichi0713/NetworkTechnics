...
🌐 イーサネットタイプとは？
イーサネットタイプ（EtherType）とは、イーサネットフレームのヘッダ部分に含まれる2バイト（16ビット）の値で、そのフレームがどんな種類のデータ（上位層プロトコル）を運んでいるかを示す識別子です。この値は16進数で表されます。

受信側の機器は、イーサネットタイプを見ることで、受け取ったデータ（ペイロード）をどのプロトコルで処理すべきか判断します。例えば、「0800」ならIP（IPv4）パケット、「0806」ならARPパケットが含まれている、といった形です。

🔍 イーサネットタイプとペイロード長
イーサネットフレームのヘッダには、歴史的にEtherTypeを示すフィールドとペイロード長を示すフィールドの両方が存在しました。

現在主流のEthernet II（DIXイーサネット）形式では、ヘッダの「タイプ」フィールドがEtherTypeを示します。一方、IEEE 802.3規格では、このフィールドはペイロード長を表していました。

両者を区別するため、EtherTypeの値は16進数の「0600」（10進数で1536）以上を用いることになっています。ペイロードの最大長は1500バイトと定められているため、このフィールドの値が1500以下であればペイロード長、1536以上であればEtherTypeとして解釈されます。
