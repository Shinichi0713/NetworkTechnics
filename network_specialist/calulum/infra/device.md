...
無線LAN（Wi‑Fi）ネットワークを識別する名前です。端末が「接続先一覧」で見ているネットワーク名がSSIDです。(itpassportsiken.com)

ESSID（Extended Service Set Identifier）
SSIDを拡張した概念で、複数のアクセスポイント（AP）を1つのネットワークとしてまとめる識別名です。(e-words.jp)

具体的な違い
BSSID：各APごとの識別子（通常はAPのMACアドレス）。(e-words.jp)
ESSID：複数APが同じESSIDを名乗ることで、建物全体などを1つの無線LANとして構成できます（ローミング可能）。(e-words.jp)
小規模ネットワークでも、現在は管理者が任意に決めたESSIDを使うのが一般的で、そのため「SSID」と言ったときESSIDを指すことが多いです。(e-words.jp)
実務上の理解
家庭用Wi‑Fiルーター：
画面に表示される「ネットワーク名（SSID）」は、実質的にESSIDです。ユーザーはSSIDだけ意識すればよいことがほとんどです。(e-words.jp)
企業やキャンパスなど：
フロアごとにAPは多数あっても、同じESSIDを設定することで、利用者は1つのSSIDに接続して歩き回れます。(e-words.jp)
要するに、

「SSID」＝無線LANの名前（広い意味）
「ESSID」＝その中でも、複数APを束ねる拡張SSID
という関係で、日常利用では区別されずに使われています。