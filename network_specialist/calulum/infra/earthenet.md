...
企業での拠点間のネットワーク通信を行うためにはどうすればよいのでしょうか？
考えてみると事業所が異なる拠点でも同じように社内ポータルを閲覧して、勤怠やレポートを報告して・・・。
この通信を支える技術が広域イーサネットです。

本日テーマ：
>広域イーサネットとはどのようなネットワークでしょうか
...
通常ネットワーク越しにパケットを転送する場合はL3技術を用いるものですが、広域イーサネットではどうでしょうか？

**「事業者が企業のIPパケットに対してL3ルーティングを行うか」という観点と、「事業者のコアネットワーク内部でL3/MPLS技術を使うか」という観点を分ける**必要があります。

結論から言えば、**Carrier Ethernet（E-Line/E-LAN/E-Tree）として提供されるサービスは、企業のIPパケットに対しては「L3ルーティングを行わない（透過的なL2転送）」のが基本**です。しかし**事業者のコア内部では、L3/MPLS技術を使ってそのL2フレームを運ぶことが一般的**です。

### 1. 企業から見た世界：「L3転送は行われていない」

MEF（Metro Ethernet Forum）が定義する標準的なCarrier Ethernetサービスは、**「L2 VPN」** として機能します。

```
【企業Aの本社】                    【企業Aの支社】
[PC: 10.1.1.2] ──┐            ┌──→ [PC: 10.1.2.2]
[ルーター]        │            │     [ルーター]
  ↓ IPパケット    │            │       ↑ IPパケット
[CEスイッチ]──→ [事業者PE]═══[事業者PE]──→ [CEスイッチ]
                 ↑___________↑
              事業者は「IPヘッダを見ない」
              イーサネットフレームをそのまま中継するだけ
```

- 企業のルーターは、遠隔地のルーターの**MACアドレス**を宛先としてフレームを送信します
- 事業者のPEは、その**イーサネットフレームをカプセル化して中継**し、出口で復元します
- **事業者はIPヘッダを見ない（原則として）** ため、企業のIPサブネット設計に干渉しません

この意味では、**「L3での転送はしていない」** という理解は正しいです。事業者はあくまで「ケーブルの延長」を提供しているに過ぎません。

### 2. しかし事業者のコア内部では「L3/MPLS」を使うことが多い

ここが重要な分岐点です。**企業のフレームを「そのまま運ぶ」ことと、事業者が「どの技術で運ぶか」は別の話**です。

実際の通信事業者のコアネットワークでは、802.1ah（PBB）だけで網全体を構築するのではなく、以下のような**L3/MPLS技術を組み合わせて**使うことが一般的です。

| 技術 | 概要 | L3的要素 |
|------|------|----------|
| **MPLS** (Multiprotocol Label Switching) | L2フレームにラベルを付けて転送。IPルーティングとは独立した経路制御が可能 | ラベル配布にBGP/OSPF-TEなどL3プロトコルを使用 |
| **VPLS** (Virtual Private LAN Service) | MPLSを使ってイーサネットの擬似ワイヤーを複数構築し、LANをエミュレート | MPLS（L3インフラ上で動作） |
| **PBB-EVPN** (Ethernet VPN) | BGPを使ってMACアドレスの到達性情報を広報。MPLSやVXLANで転送 | BGP（L3プロトコル）でMACルート交換 |
| **Segment Routing** | MPLSやIPv6の拡張ヘッダで経路を制御。IP/MPLSコアで使われる | IPv6/MPLSの制御平面 |

```
【企業から見たフレームの流れ】
[企業フレーム] → [PEがカプセル化] → [事業者コア] → [PEが復元] → [企業フレーム]

【事業者コア内部の実態（例：MPLSの場合）】
[PE] → [P] → [P] → [PE]
  ↓      ↓     ↓      ↓
MPLSラベル付きパケットとして転送
（IPルーティングテーブルとは別のラベル転送テーブル）
```

つまり、**企業のIPパケットに対して事業者が「ルーティング（L3転送）」を行うわけではありませんが、そのIPパケットが入ったイーサネットフレームを運ぶために、事業者のコアではL3/MPLS技術が使われることが多い**のです。

### 3. 「L3転送を行う」広域イーサネットサービスも存在する

また、Carrier Ethernetとは別に、事業者が**L3まで見て転送するサービス**も存在します。

| サービス | 事業者の動作 | 例 |
|----------|-------------|-----|
| **L2 VPN** (E-Line/E-LAN) | IPヘッダを見ない。L2フレームを透過転送 | Carrier Ethernet |
| **L3 VPN** (IP VPN) | IPヘッダを見て、企業ごとにVRF（仮想ルーティングテーブル）で分離してルーティング | MPLS L3 VPN、IP-VPN |
| **IP Transit** | インターネット接続。BGPで経路交換し、IPパケットをルーティング | インターネット回線 |

企業が「広域のイーサネット回線」を契約しても、それが **「L2接続サービス」なのか「L3 VPNサービス」なのか**は契約によって異なります。

## 総括

広域イーサネットが果たす役割の本質は、**「物理的に離れた場所にある複数のLANを、事業者のネットワークという『見えない回線』を通じて、あたかも同じイーサネットセグメントであるかのように接続する」** ことにあります。

以下、5つの観点から総括します。

### 1. 本質的な役割：「ケーブルの延長」

広域イーサネットの最も核心的な価値は、**「企業にとってWANの存在を意識させない」** 点です。
...
【実際の実現方式】
本社LAN → [PE] → (事業者のカプセル化・中継) → [PE] → 支社LAN
```

- 企業のルーターは、遠隔地の相手の**MACアドレス**を宛先にしてフレームを送信します
- IPアドレスの書き換えも、ルーティングテーブルの調整も**不要**です
- ブロードキャスト、マルチキャスト、IPv6、NetBIOSなど、**L2で動作するあらゆるプロトコルがそのまま通ります**

> **つまり、広域イーサネットは「地理的距離を論理的に消去するL2の透明なパイプ」です。**

### 2. 技術的本質：「構造を隠蔽するカプセル化」

事業者ネットワークが企業のフレームを中継する際、**「中身を見ずに、外側だけを自分たちの管理用に書き換える」** というカプセル化が本質的な技術です。

| 技術 | 隠蔽の仕方 | 解決した問題 |
|------|-----------|-------------|
| **802.1ad（Q-in-Q）** | 顧客VLANタグの外側に事業者VLANタグを追加 | 複数顧客のVLAN ID衝突を回避 |
| **802.1ah（PBB）** | 顧客フレーム全体を事業者用MACヘッダで包む（MAC-in-MAC） | コアスイッチのMACテーブル肥大化を防止 |
| **MPLS/VPLS** | L2フレームにMPLSラベルを付けてL3コアで転送 | 事業者コアでのスケーラブルな転送 |
| **EVPN** | BGPでMACアドレスの到達性を広報し、MPLS/VXLANで転送 | マルチホーミング・冗長化の制御平面を近代化 |

**重要なのは、「カプセル化の中身（企業のフレーム）は一切変更されない」** という点です。事業者は外側のヘッダだけを見て転送し、出口で完全に剥がします。

> **技術的本質は、「企業の通信内容（内側）」と「事業者の運用制御（外側）」を完全に分離するカプセル化アーキテクチャです。**

### 3. 進化の本質：「スケーラビリティの問題を段階的に解決した歴史」

広域イーサネットの発展は、**「普通のイーサネットを広域に出したら何が問題か」を一つずつ解決した歴史**です。

```
2000年代前半：「イーサネットを広域で使いたい」という需要発生
      ↓
問題1：VLAN IDが4094個しかない → 802.1ad（Q-in-Q）で解決
      ↓
問題2：コアスイッチが顧客MACを全て学習しなければならない → 802.1ah（PBB）で解決
      ↓
問題3：STPによる帯域の無駄、障害復旧の遅さ → MPLS/EVPN/Segment Routingで解決
      ↓
現在：「L2 VPNとしての柔軟性」と「L3/MPLSコアとしてのスケーラビリティ」を統合
```

> **進化の本質は、「イーサネットの利便性（シンプルさ・低コスト・統一インターフェース）」を「キャリアグレードのスケーラビリティ・信頼性・管理性」と融合させ続けたことです。**

### 4. ビジネス的本質：「LANとWANの境界をなくしたサービス化」

技術的な役割を超えて、広域イーサネットは**企業ネットワークの運用パラダイムを変えました**。

| 従来（専用線/フレームリレー/ATM） | 広域イーサネット |
|-----------------------------------|------------------|
| LAN機器とWAN機器が異なる | イーサネットポートをそのまま使える |
| 帯域変更に物理的な工事・長期契約が必要 | 論理的に帯域を変更可能（10M→1Gなど） |
| プロトコル変換が必要（LAN←→WAN） | プロトコル変換なし（L2透過） |
| 拠点追加のたびに複雑な設定 | E-LANでany-to-anyの論理LANを構築可能 |

> **ビジネス的本質は、「LANのシンプルさと経済性をWAN世界に持ち込み、ネットワーク接続を『物理資産』から『論理サービス』へと変換した」ことです。**

### 5. 現代における位置づけ：「SD-WANやクラウド時代の基盤」

広域イーサネットは古い技術ではなく、**現代のネットワークアーキテクチャの基盤**として進化し続けています。

```
【現代の企業ネットワーク】
[支社A]──┐
[支社B]──┼──→ [SD-WAN CPE] ──→ [インターネット/IP-VPN] ──→ [クラウド/SaaS]
[支社C]──┘         ↑
              その下には「広域イーサネットの技術」が使われることも多い
              （事業者のL2 VPN基盤、EVPN over MPLSなど）
```

- **SD-WAN**：多くの場合、**事業者のL2/L3 VPN基盤**の上にオーバーレイで構築されます
- **クラウド接続（AWS/Azure Direct Connectなど）**：事業者の広域イーサネットからクラウドのL2接続へダイレクトに接続する形態が一般的
- **5G/モバイルバックホール**：基地局とコア間の接続にもCarrier Ethernet技術が使われています

> **現代における本質は、「クラウド・SD-WAN・5G時代においても、『拠点間のL2透過接続』という基盤的役割を担い続けている」ことです。**
